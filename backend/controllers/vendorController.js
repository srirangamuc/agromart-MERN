...
// Use this code inside an async function or route
// replace with actual ID

            // --- Without Index ---
            console.time('Product Query - No Index');
            const productsNoIndex = await Vendor.find({ vendor: vendorId })
                .hint({ $natural: 1 }) // forces collection scan
                .sort({ timestamp: 1 });
            console.timeEnd('Product Query - No Index');

            // --- With Index ---
            console.time('Product Query - With Index');
            const products= await Vendor.find({ vendor: vendorId })
                .sort({ timestamp: 1 }); // uses index if available
            console.timeEnd('Product Query - With Index');
...
// Function to rate vendor
    async rateVendor(req, res) {
        try {
            ...
            // Check if vendor already rated in this purchase
            const existingRatingIndex = purchase.vendorRatings.findIndex(
                (vr) => vr.vendor.toString() === vendorId
            );
            ...
            // Save updated purchase document
            await purchase.save();

            // ✅ FIXED: Don't overwrite the model name!
            let vendorRating = await VendorRating.findOne({ vendor: vendorId });
